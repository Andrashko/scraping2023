...
class InstagramSpider(scrapy.Spider):
    ...
    def login(self, driver, wait):
        ...
        username_input.send_keys("Uzhnu2024")
        username_password = driver.find_element(By.XPATH, '//input[@name="password"]')
        username_password.send_keys("Uzhnu2024Password")   
        login_button = driver.find_element(By.XPATH, '//button[@type="submit"]')
        login_button.click()
        wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//div[contains(text(),'Не зараз')]")))
        later_button = driver.find_element(By.XPATH, "//div[contains(text(),'Не зараз')]")
        later_button.click()
